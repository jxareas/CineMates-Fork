...
class SectionRecyclerViewAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SectionViewHolder {
        return SectionViewHolder(parent help ListItemSectionBinding::inflate)
    }
    ...
    override fun getItemViewType(position: Int): Int {
        if (dataList[position].genericType == Movie::class.java) {
            return MOVIE
        } else if (dataList[position].genericType == Person::class.java || dataList[position].genericType == Cast::class.java) {
            return PERSON
        }
        return -1
    }

   inner class SectionViewHolder (val binding: ListItemSectionBinding) :
